...
export function reconcileChildren(returnFiber, children) {
  ...
  // 还有一种情况，也属于初次渲染：旧节点遍历完了，新节点还有剩余，那么这些新节点就是属于初次渲染
  // 4、处理新旧节点都还有剩余的情况
  // (1) 将剩下旧节点放入到一个 map 结构里面，方便之后使用
  // (2) 遍历剩余的新节点，通过新节点的 key 去map里面及进行查找，看有没有能够复用的旧节点，如果有，拿来复用，并且会 map 中删除对应的旧节点。
  // 5、整个新节点遍历完成后，如果 map 中，还有剩余的旧节点，这些旧节点也就没有用了，直接删除即可。
  ...
  for (; oldFiber && i < newChildren.length; i++) {
    ...
    // 这里有一种情况
    // old 一开始是 1 2 3 4 5，进行了一些修改，现在只剩下 5和4
    // old >> 5(4) 4(3) 框号内是索引
    // new >> 4(3) 1 2 3 5(4)
  ...
  // 第二步骤：检查 newChildren 是否完成了遍历
  // 从上面的 for 循环出来，有两种情况
  // 1、oldFiber为null，说明是初次渲染。
  // 2、i === newChildren.length 说明是更新，是遍历完了才出来的
  if (i === newChildren.length) {
    // 如果还剩余有旧的 fiber 节点，那么就需要将其删除掉
    deleteReaminChildren(returnFiber, oldFiber);
    return;
  }
  ...
  for (; i < newChildren.length; i++) {
    ...
    // 第五步骤：整个新节点遍历完成后，如果 map 中，还有剩余的旧节点，这些旧节点也就没有用了，直接删除即可。
