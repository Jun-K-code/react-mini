const getParentDOM = (wip) => {
  ...
  while (temp) {
    ...
    // 因为该 fiber 节点可能是一个函数组件 或者 类组件、Fragment
    temp = temp.return;
  }
};

const commitNode = (wip) => {
  // 1、首先第一步，我们需要获取该 fiber 所对应的父节点的 DOM 对象
  const parentNodeDOM = getParentDOM(wip.return);
  console.log('测试commitNode parentNodeDOM', parentNodeDOM);
  console.log('测试commitNode wip.stateNode', wip.stateNode);
